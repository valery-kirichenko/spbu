    ...
    protected void init(VaadinRequest vaadinRequest) {
        ...
        createCreditButton.addClickListener(clickEvent -> {
            ...
            try {
                ...
                List<Credit> creditUpdate = template.query("SELECT * FROM credits WHERE clientID =" + id, new CreditRowMapper());
                creditGrid.setItems(creditUpdate);
            }
            catch (IllegalArgumentException e) {
                new Notification(
                        e.toString(),
        ...
        deleteCreditButton.addClickListener((Button.ClickListener) clickEvent -> {
            ...
            List<Credit> creditUpdate = template.query("SELECT * FROM credits WHERE clientID =" + Integer.parseInt(createCreditClientID.getValue()), new CreditRowMapper());
            creditGrid.setItems(creditUpdate);
        });
        ...
        layout.addComponents(clientGrid, createClientLayout, createCreditLayout, deleteLayout, creditGrid);
        setContent(layout);
