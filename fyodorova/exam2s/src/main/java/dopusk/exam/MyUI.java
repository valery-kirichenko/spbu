...
public class MyUI extends UI {
    ...
    @Autowired
    private ClientControlImpl clientController;

    @Autowired
    private CreditControlImpl creditController;
    ...
    protected void init(VaadinRequest vaadinRequest) {
        ...
        clientGrid.addColumn(Clients::getmName).setCaption("MiddleName");
        clientGrid.addColumn(Clients::getNewPass).setCaption("Passport");
        clientGrid.addColumn(Clients::getOldPass).setCaption("OldPassport");
        clientGrid.getEditor().setEnabled(true);

        Grid<Credits> creditGrid = new Grid<>();
        creditGrid.setSizeFull();
        creditGrid.addColumn(Credits::getCredId).setCaption("Credit ID ").setWidth(100);
        creditGrid.addColumn(Credits::getClId).setCaption("Client ID").setWidth(100);
        ...
        try {
            readerInterface.mainMerge();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        List<Credits> creditList = readerInterface.getCreditsList();
        List<Clients> clientList = readerInterface.getClientsList();
        ...
        buttonAddCredit.addClickListener(clickEvent -> {
            if (IdClientSelected.getValue() != null) {
                ...
                credit.setClId(Integer.parseInt(IdClientSelected.getValue()));
        ...
        buttonAddClient.addClickListener(clickEvent -> {
            ...
            newClient.setmName(clientMidName.getValue());
            newClient.setPh(clientPhone.getValue());
            newClient.setNewPass(clientPassport.getValue());
            newClient.setData(clientDate.getValue());
            newClient.setOldPass(clientOldPassport.getValue());

            if (newClient.getId() == null || newClient.getName() == null || newClient.getSurName() == null ||
                    newClient.getOldPass() == null ||
                    newClient.getmName() == null || newClient.getPh() == null ||
                    newClient.getNewPass() == null)
       ...
       ;
        ...
        horlayout.addComponents(buttonAddClient, buttonAddCredit, buttonDodjers, buttonNotDodjers,
                buttonAllClients, IdClientSelected);
